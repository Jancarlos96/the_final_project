...
GameScene::GameScene()
{
    ...
    scale = 0.3;
    crear_Mapa();
    set_background(":/Sprite/game_objects/bloque.png", 0.25);
}
...
void GameScene::Agregar_Jugador()
{
    p_pal = new Jugador();
...
bool GameScene::ComprobarColision(QGraphicsItem* item, short dir, int Ancho, int Alto)
{
    ...
    // if (bl_mapa[qFloor(x / (Ancho*scale))][qFloor(y / (Alto*scale))]->is_muro() ||
    //     bl_mapa[qFloor(x1 / (Ancho*scale))][qFloor(y1 / (Alto*scale))]->is_muro()) {
    //     move = false;
    // }
    return move;
}

void GameScene::set_background(QString name, float scale)
{
    QImage k;
    QBrush n;

    k.load(name);
    //k = k.scaled(k.width(),k.height());
    //n.setTextureImage(k);
